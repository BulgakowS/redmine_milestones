class Milestone < ActiveRecord::Base
  ...
  scope :opened, :conditions => {:status => %w(open locked)}
  ...
  scope :orphaned, :conditions => ['parent_milestone_id IS NULL or parent_milestone_id = ?', 0]
  scope :versionless, :conditions => ['version_id IS NULL OR version_id = ?', 0]
